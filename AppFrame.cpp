					 ...
					 CWindow(Name, PosX, PosY, DimX, DimY), Degree(0), mScale(100.0f)
{
	...
	ModelloPtr->Copy3dsFile("tpot.3ds","embed_tpot.3ds");
	ModelloPtr->ProcessFile("tpot.3ds","embed_tpot.3ds");
	...
	for (int i = 0; i < nobj; i++)
	{
		...
		//algorithm for sending 3d mesh watermark.
		ObjectPtr->embeddingAlgo();
		

		// dump the embedded vertices in 3ds file
		//ModelloPtr->write3dsfile();
		ModelloPtr->EmbedFile("tpot.3ds","embed_tpot.3ds",ObjectPtr);
		//ObjectPtr->write3dsfile("tpot.3ds","embed_tpot.3ds");
