...
public:
// Constructor
	C3dsFileLoader() : C3dsFileFormat(), PC(0), OPC(0),mFileSize(0) {}
// Methods
	void		Copy3dsFile(string IpFileName,string OpFileName);
	void		ProcessFile(string IpFileName,string OpFileName);
	void		EmbedFile(string IpFileName,string OpFileName,C3DObject *objptr);
private:
	void		movePC(long forward);
	bool		ParseChunks();
	bool		EmbedParseChunks(C3DObject *objptr);
	bool		EmbedChunks();
	bool		ReadBlock(char *dest, long size);
	bool		ReadEmbeddBlock(char *dest, long size);
	bool		SetWriteChunkCntr();
	bool		WriteBlock(char *dest, long size);
	float		ReadRealNumber();
	bool		SkipChunk(chunk3ds &chunk);
	bool		SkipEmbeddedChunk(chunk3ds &chunk);
	void		FileSize();
	bool		isOk();
	...
	void		ReadObjBlock();
	void		ReadVertList();
	void		WriteVertList(C3DObject *objptr);
	void		ReadFaceList();
	void		ReadFaceMat();
	void		ReadMapList();
	void		ReadSmoothList();
